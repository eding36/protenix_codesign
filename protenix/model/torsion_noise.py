...
def apply_torsion_noise(
    ...
    Args:
        coords: ``[..., N_atom, 3]``
        torsion_pivot/depth/atom/atom_id: values generated by torsion_index_to_tensors() fxn. bookkeeping indices to help figure out where to perform angular noising.
    ...
    #for each rotatable side chain bond, calculate (for all the atoms it moves) how far they are from the rotational axis formed by the bond. This is the lever variable
    ...
    lever = (sums / cnts.clamp(min=1.0)).sqrt().clamp(min=0.3)  # [..., n_torsion] #describes on average how much a chi's connected atoms will move

    theta_scale = (THETA_PER_SIGMA * sigma[..., None] / lever).clamp(max=max_angle) #scale theta using sigma*THETA_PER_SIGMA/lever
    angles = torch.randn(
